...
class BilibiliDownloader(Downloader, ABC):
    ...
    def _write_netscape_cookie_file(self) -> Optional[str]:
        ...
        Bilibili 近期加强了 API 反爬（WBI 签名端点普遍返回 412），
        但网页内嵌的 __playinfo__ 仍包含可用流地址。
        yt-dlp 仅在 is_logged_in 为 True 时提取 __playinfo__，
        因此注入一个虚拟 SESSDATA 以绕过该检查，避免调用被封锁的 API。
        """
        ...
        # 注入虚拟 SESSDATA 以触发 yt-dlp 走 __playinfo__ 路径
        if not has_sessdata:
            lines.append(".bilibili.com\tTRUE\t/\tTRUE\t0\tSESSDATA\tfake\n")

        tmp = tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False, encoding='utf-8')
        tmp.writelines(lines)
        tmp.close()
        logger.info("已生成 B站 Netscape Cookie 文件: %s (条目: %d, 有真实SESSDATA=%s)",
                     tmp.name, len(lines) - 1, has_sessdata)
        return tmp.name
