...
async def export_pdf(data: ExportPDFRequest):
    ...
    if isinstance(markdown, list):
        if data.ver_id:
            for v in markdown:
                if v.get("ver_id") == data.ver_id:
                    markdown = v.get("content", "")
                    break
        elif markdown:
            markdown = markdown[-1].get("content", "")

    if not markdown:
        return R.error(msg="笔记内容为空")

    title = result.get("audioMeta", {}).get("title", "笔记")

    # 生成 PDF
    exporter = ExportUtils()
    pdf_path = exporter.export("pdf", title, markdown)
