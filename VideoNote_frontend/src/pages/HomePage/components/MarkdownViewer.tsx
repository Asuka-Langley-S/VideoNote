        ...
        return (
          <span className="origin-link my-2 inline-flex">
            <a
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-primary-light px-3 py-1 text-sm font-medium text-primary transition-colors hover:bg-primary/10"
              {...props}
            >
...
const MarkdownViewer: FC<MarkdownViewerProps> = memo(({ status }) => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const res = await fetch(`${baseURL.replace(/\/$/, '')}/export_pdf`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ task_id: task.id, ver_id: currentVerId }),
      })
      ...
      const name = task.audioMeta?.title || '笔记'
      const link = document.createElement('a')
      link.href = URL.createObjectURL(blob)
    ...
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center space-y-3 text-neutral-500">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-xs text-neutral-500">支持哔哩哔哩、抖音、快手等视频平台</p>
        </div>
      </div>
    )
  }
          ...
          {selectedContent && selectedContent !== 'loading' && selectedContent !== 'empty' ? (
            ...
            <div className="flex h-full w-full items-center justify-center">
              <div className="w-[300px] flex-col justify-items-center">
                ...
                <p className="text-xs text-neutral-500">支持哔哩哔哩、抖音、快手等视频网站</p>
              </div>
            </div>
          )}
        </div>
